...
def _update_lineas_sheet(wb, excz_dir, prefix, currency_fmt, border, bold_font):
    ...
    if not df.empty:
        ...
        for key in ["cantidad", "ventas", "costos", "renta", "utili"]:
        ...
        totals = totals.drop_duplicates(
            subset=["linea", "grupo", "cantidad", "ventas", "costos", "renta", "utili"],
            keep="first",
        )

        totals["utili_calc"] = totals["ventas"] - totals["costos"]
        totals["utili"] = totals["utili"].where(~totals["utili"].isna(), totals["utili_calc"])
        ...
        totals.drop(columns=["utili_calc", "renta_calc"], inplace=True)
